 ...
 *
 * Forked from mdbassit/Coloris 0.25.0 (MIT) for paper_cover. Local patches, each marked [FORK]:
 * 1. The RGB and HSL format tabs render three separate numeric fields (R/G/B, H/S/L) instead
 *    of one CSS-string text field, so users can type exact channel values directly.
 * 2. pickColor() always emits a hex string regardless of which format tab is active, since the
 *    RGB/HSL tabs are an editing convenience only — hosts consuming this widget store colors
 *    as plain hex.
 */
...
((window, document, Math, undefined) => {
  ...
  let container, picker, colorArea, colorMarker, colorPreview, colorValue, clearButton, closeButton,
      ...
      colorAreaDims = {},
      rgbFields, hslFields, fieldR, fieldG, fieldB, fieldH, fieldS, fieldL; // [FORK]
  ...
  const settings = {
    ...
    format: 'hex',
    formatToggle : false,
    swatchGroups: [], // [FORK] {label, colors}[] — see the 'swatchGroups' case in configure()
    swatchesOnly: false,
    alpha: true,
    forceAlpha: false,
    focusInput: true,
    ...
    for (const key in options) {
      switch (key) {
        ...
        case 'formatToggle':
          ...
          break;
        // [FORK] Replaces upstream's single flat 'swatches' list with labeled groups (e.g.
        // "Mine" / "Auto") each on their own wrapping row, per {label, colors} entries
        case 'swatchGroups':
          if (Array.isArray(options.swatchGroups)) {
            const swatchesContainer = getEl('clr-swatches');
            let buttonIndex = 0;

            // Clear current swatch groups
            swatchesContainer.textContent = '';

            options.swatchGroups.forEach(group => {
              if (!group.colors || !group.colors.length) {
                return; // skip empty groups rather than showing a bare label
              }
              ...
              groupEl.className = 'clr-swatch-group';
              labelEl.className = 'clr-swatch-group-label';
              labelEl.textContent = group.label;
              rowEl.className = 'clr-swatch-row';
              rowEl.setAttribute('role', 'group');
              rowEl.setAttribute('aria-label', group.label);

              group.colors.forEach(swatch => {
              ...
              swatchesContainer.appendChild(groupEl);
            });

            settings.swatchGroups = options.swatchGroups.slice();
          }
          break;
        case 'swatchesOnly':
  ...
  function openPicker(event) {
    ...
    if (settings.focusInput || settings.selectInput) {
      ...
      // [FORK] Mirroring currentEl's own selection made sense upstream, where currentEl is a
      // real visible text field the user just clicked into. Here it's always our invisible
      // readonly trigger input, whose selection is always (0,0) — that put the cursor before
      // the "#" every time instead of after the value. Put it at the end instead.
      const end = colorValue.value.length;
      colorValue.setSelectionRange(end, end);
    }
  ...
  function pickColor(color) {
    // [FORK] Fall back to the hex representation of the active color rather than colorValue's
    // current text (which may hold an "rgb(...)"/"hsl(...)" string while those tabs are active)
    // — hosts consuming this widget always expect a stable hex string
    color = color !== undefined ? color : RGBAToHex(currentColor);
  ...
  function updateColor(rgba = {}, hsva = {}) {
    ...
    // [FORK] No separate preview circle — the value field(s) themselves show the chosen color
    // as their own background, with auto white/black text for contrast
    const text_color = pickContrastColor(currentColor.r, currentColor.g, currentColor.b);
    ...
    // [FORK] RGB/HSL show three separate numeric fields instead of one CSS-string field —
    // toggle which field group is visible and populate it from the current color
    switch (format) {
      case 'hex':
        colorValue.value = hex;
        colorValue.style.display = '';
        rgbFields.style.display = 'none';
        hslFields.style.display = 'none';
        colorValue.style.backgroundColor = hex;
        colorValue.style.color = text_color;
        break;
      case 'rgb':
        fieldR.value = Math.round(currentColor.r);
        fieldG.value = Math.round(currentColor.g);
        fieldB.value = Math.round(currentColor.b);
        colorValue.style.display = 'none';
        rgbFields.style.display = 'flex';
        hslFields.style.display = 'none';
        rgbFields.style.backgroundColor = hex;
        rgbFields.style.color = text_color;
        break;
      case 'hsl':
        const hsla = HSVAtoHSLA(currentColor);
        fieldH.value = Math.round(hsla.h);
        fieldS.value = Math.round(hsla.s);
        fieldL.value = Math.round(hsla.l);
        colorValue.style.display = 'none';
        rgbFields.style.display = 'none';
        hslFields.style.display = 'flex';
        hslFields.style.backgroundColor = hex;
        hslFields.style.color = text_color;
        break;
    }
  ...
  /**
   * [FORK] Pick white or black, whichever contrasts more against an RGB color (ITU-R BT.601
   * luma) — matches the app's own contrast_color() in widget/src/svg_utils.ts.
  ...
  function init() {
    ...
    // [FORK] Three-field RGB/HSL groups, shown instead of clr-color while those tabs are active
    '<div id="clr-rgb-fields" class="clr-field-group">'+
      '<label>R<input id="clr-r" type="number" min="0" max="255" step="1"></label>'+
      '<label>G<input id="clr-g" type="number" min="0" max="255" step="1"></label>'+
      '<label>B<input id="clr-b" type="number" min="0" max="255" step="1"></label>'+
    '</div>'+
    '<div id="clr-hsl-fields" class="clr-field-group">'+
      '<label>H<input id="clr-h" type="number" min="0" max="360" step="1"></label>'+
      '<label>S<input id="clr-s" type="number" min="0" max="100" step="1"></label>'+
      '<label>L<input id="clr-l" type="number" min="0" max="100" step="1"></label>'+
    '</div>'+
    ...
    // [FORK] Reference the RGB/HSL field groups
    rgbFields = getEl('clr-rgb-fields');
    hslFields = getEl('clr-hsl-fields');
    fieldR = getEl('clr-r');
    fieldG = getEl('clr-g');
    fieldB = getEl('clr-b');
    fieldH = getEl('clr-h');
    ...
    addListener(colorValue, 'change', event => {
      // [FORK] Don't require the leading '#' — auto-prefix it when missing rather than
      // silently falling back to black (strToRGBA's canvas parse rejects a bare hex digit
      // string with no '#', since it isn't valid CSS on its own)
    ...
    // [FORK] RGB/HSL field groups: assemble a CSS color string from the three current values
    // and feed it through the existing string-based pipeline (setColorFromStr), so none of the
    // RGB<->HSL math needs to be duplicated here. Alpha stays driven by the alpha slider.
    //
    // Committing on 'focusout' of the whole group (not 'change' on each field) is deliberate:
    // updateColor() re-populates all three fields from the round-tripped color on every commit,
    // and that round trip isn't lossless (e.g. hue collapses to 0 once saturation rounds to 0).
    // Committing per-field would let an in-progress edit to one field get silently overwritten
    // by a round trip computed from the *other two* fields' still-stale values before the user
    // finishes typing. Waiting for the group to lose focus means every field the user touched
    // has its typed value read exactly once, together, with no intermediate round trip in between.
    addListener(rgbFields, 'focusout', event => {
      if (rgbFields.contains(event.relatedTarget)) return; // still tabbing within R/G/B
      if (currentEl || settings.inline) {
        setColorFromStr(`rgba(${fieldR.value || 0}, ${fieldG.value || 0}, ${fieldB.value || 0}, ${currentColor.a})`);
        pickColor();
      }
    });

    addListener(hslFields, 'focusout', event => {
      if (hslFields.contains(event.relatedTarget)) return; // still tabbing within H/S/L
      if (currentEl || settings.inline) {
        setColorFromStr(`hsla(${fieldH.value || 0}, ${fieldS.value || 0}%, ${fieldL.value || 0}%, ${currentColor.a})`);
        pickColor();
      }
    });
...
// [FORK] ES module export so bundlers (Vite) can `import Coloris from './coloris.js'` — see
// coloris.d.ts for its type
export default window.Coloris;
