...
// Samples dominant colors from key regions of a background image (for auto color matching —
// see image_regions_cache.ts). front_top/front_bottom come from the top/bottom slivers (see
// TOP_FRACTION/BOTTOM_FRACTION below) of the whole raw image — correct whenever the image is
// confined to the front panel. front_top_full/front_bottom_full, back, and spine instead locate
// the front/back panels and spine strip proportionally within the same image, for when it's a
// full-wrap spread (bg_image_coverage 'full') — both interpretations are sampled together so
// switching coverage never needs a recompute.

import type {Region} from 'printing-services'
import type {RegionStats, ImageRegions} from 'bookcover-web'
...
function analyze_rect(data:Uint8ClampedArray, img_w:number, img_h:number,
...
/** Sample a mm-based panel Region (already scaled to px) into its top/bottom text regions —
 *  the top TOP_FRACTION and bottom BOTTOM_FRACTION of its height, not a 50/50 split */
 ...
 * (front_top_full/front_bottom_full/back/spine all come back null).
 */
export async function analyze_image_regions(file:File, dims:CoverDims | null):Promise<ImageRegions> {
    ...
    const front_top = analyze_rect(data, w, h, {x: 0, y: 0, w, h: h * TOP_FRACTION})
    const front_bottom = analyze_rect(data, w, h, {x: 0, y: h * (1 - BOTTOM_FRACTION), w, h: h * BOTTOM_FRACTION})
    ...
    if (dims) {
        ...
        const to_px = (region:Region) => region_to_px(region, total_w, total_h, w, h)
        const front_split = split_top_bottom(data, w, h, to_px(dims.cover_region_front))
        front_top_full = front_split.top
        front_bottom_full = front_split.bottom
        back = analyze_rect(data, w, h, to_px(dims.cover_region_back))
        spine = analyze_rect(data, w, h, to_px(dims.cover_region_spine))
    }
