...
import type {BuiltinBg} from './generated/builtin_bg_regions.js'
...
/** Path (relative to the assets root) of the preview-sized copies of the builtin backgrounds —
 *  2700px wide, ~380KB, for rendering previews without downloading the original. Join a
 *  builtin's filename onto it; the filename is the same ID as the original's. Generated by
 *  .bin/gen_bg_thumbnails, whose header explains the size */
export const BG_PREVIEW_DIR = 'backgrounds/previews_2700/'

/** Baked metadata for a builtin background (assets/backgrounds/<filename>) — its original's pixel
 *  size and its precomputed ImageRegions (see builtin_bg_regions.ts for how/why these are baked
 *  in). Returns null for anything not in the table */
export function get_builtin_bg(filename:string):BuiltinBg | null {
    return Object.hasOwn(BUILTIN_BG_REGIONS, filename) ? BUILTIN_BG_REGIONS[filename] : null
}

/** Precomputed ImageRegions for a builtin background, looked up by its filename (its ID) alone —
 *  so they apply whatever copy of it is being rendered, the original or the preview-sized one in
 *  BG_PREVIEW_DIR. They include the full-wrap regions (sampled against a 6x9 reference cover).
 *  Returns null for anything not in the table, in which case callers fall back to
 *  analyze_pixel_regions() against the actual decoded image */
export function get_builtin_bg_regions(filename:string):ImageRegions | null {
    return get_builtin_bg(filename)?.regions ?? null
}
