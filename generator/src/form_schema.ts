 ...
 *  Color fields carry only explicit user overrides (converted from the form's raw hex to the
 *  schema's hsl() string format) — anything left unset stays unset here, for resolve_colors()
 *  (generator/src/design.ts, run downstream inside build()) to fill in from bg_color/spine_color
 *  and, when a background image is active, the sampled image regions. */
export function build_schema(
    ...
    return {
        ...
        blurb_bg_color: form.blurb_bg_color === undefined
            ? undefined  // auto-derived downstream by resolve_colors()
