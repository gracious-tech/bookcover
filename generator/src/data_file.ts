...
export function build_data_file(
    ...
    // Coil/wire bindings need extra clearance on the spine-adjacent edge of each panel for the
    // punched holes (2mm gap + 5mm hole width). Kept in sync manually with 3d/src/geometry.ts's
    // HOLE_LEFT_GAP_MM/HOLE_WIDTH_MM — the two packages don't share a workspace.
    const is_spiral = schema.binding_type === 'paperback_coil' || schema.binding_type === 'paperback_wire'
    const spiral_margin_mm = is_spiral ? 7 : 0
    ...
    // Barcode region (x from canvas left, y from trim bottom) — pulled clear of the spiral margin
    lines.push(mmval('barcode_x', dims.cover_region_barcode.x.toNumber() - spiral_margin_mm))
    ...
    lines.push(mmval('spiral_margin', spiral_margin_mm))
    // Derived content areas — front narrows on the spine (left) side only, for spiral holes
    lines.push(mmval('front_content_w', face_width_mm - front_margin_mm * 2 - spiral_margin_mm))
    ...
    // Blurb width (% of face_width, clamped to back content width). The spiral margin is
    // subtracted here too (not just from the centering dx in cover.typ) so a maxed-out
