...
// i18n namespace (see widget/src/locales/eng.ts) for subheadings in the icon picker dialog.
// Metadata only — the SVG strings live in preset_icons_svg.ts so importing this list (which
// the widget's picker needs on the main thread) doesn't drag the SVG payload along, matching
// the patterns.ts / patterns_svg.ts split. This is the source list bundled by
// .bin/fetch_preset_icons into generator/preset_icon_svgs/ (committed) — see that script.
